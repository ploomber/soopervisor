...
@pytest.mark.parametrize(
    "preset, operator_class, needs_docker",
    [
...
def test_airflow_export_sample_project(
    ...
    exporter = AirflowExporter.new(
        path_to_config="soopervisor.yaml", env_name="serve", preset=preset
    )
    ...
    assert {n: t.upstream_task_ids for n, t in dag.task_dict.items()} == {
        "raw": set(),
        "clean": {"raw"},
        "plot": {"clean"},
    }
    ...
    assert (Path("serve", "Dockerfile").exists() and needs_docker) or (
        not Path("serve", "Dockerfile").exists() and not needs_docker
    )
...
def test_export_airflow_callables(
    monkeypatch,
    mock_docker_calls_callables,
    tmp_callables,
    no_sys_modules_cache,
    skip_repo_validation,
    mode,
    ...
    exporter = AirflowExporter.new(path_to_config="soopervisor.yaml", env_name="serve")
