...
from soopervisor.abc import AbstractExporter, AbstractConfig, AbstractDockerConfig
...
def test_error_if_missing_environment_lock_yml(method, tmp_sample_project):
    ...
    with pytest.raises(ClickException) as excinfo:
        getattr(ConcreteExporter, method)("soopervisor.yaml", env_name="some_env")
...
def test_error_if_dag_fails_to_initialize(method, tmp_sample_project):
    ...
    assert "Could not find dag spec with name pipeline.yaml" in str(excinfo.value)
...
"""
    ...
    expected = (
        "'soopervisor.yaml' does not contain a target " "environment named 'some_env'"
    )
...
"""
    ...
    expected = (
        "A target environment named 'env' already exists in "
