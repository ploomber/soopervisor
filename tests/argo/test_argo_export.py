...
def test_export(
    ...
    exporter = ArgoWorkflowsExporter.new(
        path_to_config="soopervisor.yaml", env_name="serve"
    )
    exporter.add()
    ...
    load_tasks_mock.assert_called_once_with(
        cmdr=ANY, name="serve", mode=mode, lazy_import=False, task_name=None
    )
    ...
    cmd = "ploomber task {{inputs.parameters.task_name}} --entry-point " + str(
    ...
    assert run_task_template["script"]["image"] == "your-repository/name:0.1dev"
    assert run_task_template["name"] == "run-task"
    assert spec["metadata"]["generateName"] == "my-project-"
    assert all([set(dag[t["name"]].upstream) == set(t["dependencies"]) for t in tasks])
    ...
    assert all(
        [
            t["arguments"]["parameters"][0] == {"name": "task_name", "value": t["name"]}
            for t in tasks
        ]
    )
...
def test_custom_volumes(
    mock_docker_my_project, backup_packaged_project, skip_repo_validation
):
    exporter = ArgoWorkflowsExporter.new(
        path_to_config="soopervisor.yaml", env_name="serve"
    )
    ...
    spec["serve"]["mounted_volumes"] = [
        {
            ...
            "spec": {"persistentVolumeClaim": {"claimName": "someName"}},
        }
    ]
    ...
    # reload exporter
    ArgoWorkflowsExporter.load(
        path_to_config="soopervisor.yaml", env_name="serve"
    ).export(mode="incremental", until=None)

    spec = yaml.safe_load(Path("serve/argo.yaml").read_text())

    assert spec["spec"]["volumes"] == [
        {"name": "nfs", "persistentVolumeClaim": {"claimName": "someName"}}
    ]

    run_task_template = spec["spec"]["templates"][0]
    assert run_task_template["script"]["volumeMounts"] == [
        {"name": "nfs", "mountPath": "/mnt/nfs", "subPath": "some_subpath"}
    ]


def test_export_with_null_repository(
    mock_docker_my_project, backup_packaged_project, skip_repo_validation, capsys
):
    exporter = ArgoWorkflowsExporter.new(
        path_to_config="soopervisor.yaml", env_name="serve"
    )
