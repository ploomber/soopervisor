...
from soopervisor.airflow.export import AirflowExporter, commons as airflow_commons
from soopervisor.argo.export import ArgoWorkflowsExporter, commons as argo_commons
from soopervisor.aws.batch import AWSBatchExporter, commons as batch_commons
from soopervisor.kubeflow.export import KubeflowExporter, commons as kubeflow_commons
...
def test_dockerfile_when_no_setup_py(CLASS_, tmp_sample_project, no_sys_modules_cache):
...
def test_add_creates_necessary_files(
    CLASS_, files, tmp_sample_project, no_sys_modules_cache
):
    exporter = CLASS_.new(path_to_config="soopervisor.yaml", env_name="serve")
    exporter.add()
...
def test_checks_the_right_spec(
    monkeypatch,
    mock_docker_sample_project_serve,
    tmp_sample_project,
    no_sys_modules_cache,
    skip_repo_validation,
    CLASS,
...
def test_checks_the_right_spec_pkg(
...
def test_validates_repository(mock_docker_sample_project, tmp_sample_project, CLASS):
    exporter = CLASS.new(path_to_config="soopervisor.yaml", env_name="serve")
    exporter.add()
    ...
    assert str(excinfo.value) == (
        "Invalid repository 'your-repository/name' "
...
def test_docker_local_lib_import(
    ...
    try:
        client.containers.run(
            "sample_project:latest", "python -c 'from lib import package_b;'"
        )
...
def test_skip_docker(
    ...
    exporter = CLASS_.new(path_to_config="soopervisor.yaml", env_name=f"serve_{mode}")
    exporter.add()
