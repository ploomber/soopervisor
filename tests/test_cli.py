...
def test_add_unknown_backend():
    ...
    result = runner.invoke(
        cli, ["add", "something", "--backend", "unknown"], catch_exceptions=False
    )
...
def test_export_missing_soopervisor_yaml(tmp_empty):
    runner = CliRunner()
    result = runner.invoke(cli, ["export", "something"], catch_exceptions=False)

    expected = (
        "Error: Expected a 'soopervisor.yaml' file in the "
...
def test_p_home_exists_tar(
    args, backend, tmp_sample_project, monkeypatch, monkeypatch_external
):
    ...
    with open("soopervisor.yaml", "w") as file:
        yaml.dump(yml, file)
    file.close()
    result = runner.invoke(cli, ["export", "-i", "-s", "serve"], catch_exceptions=False)
...
def test_error_if_invalid_preset(tmp_sample_project, backend, preset):
    ...
    assert res.exit_code
    expected = (
        "Error: Preset 'invalid' is not a valid value for "
...
def test_sample_project_no_args(args, backend, tmp_sample_project, monkeypatch):
    ...
    exporter_.load.assert_called_once_with(
        "soopervisor.yaml", env_name="serve", lazy_import=False
    )
    exporter_.load().export.assert_called_once_with(
        mode="incremental",
        until=None,
        skip_tests=False,
        skip_docker=False,
...
def test_sample_project(
    args, args_export, mode, backend, tmp_sample_project, monkeypatch, task_name
):
    runner = CliRunner()
    result = runner.invoke(cli, args, catch_exceptions=False)
    assert result.exit_code == 0
    ...
    result = runner.invoke(
        cli, ["export", "serve"] + args_export, catch_exceptions=False
    )
    assert result.exit_code == 0
...
def test_skip_tests(args, backend, tmp_sample_project, monkeypatch):
    ...
    result = runner.invoke(
        cli, ["export", "serve", "--skip-tests"], catch_exceptions=False
    )
    assert result.exit_code == 0
...
def test_ignore_git(args, backend, tmp_sample_project, monkeypatch_external):
    ...
    if backend != Backend.aws_batch:
        ...
        new = f.read_text().replace("your-repository/name", "your-repository/something")

    f.write_text(new)

    # test export with --ignore-git
    result = runner.invoke(
        cli, ["export", "serve", "--ignore-git"], catch_exceptions=False
    )
    assert result.exit_code == 0
