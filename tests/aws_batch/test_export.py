...
def index_job_name_by_id(jobs_info):
    ...
    return merge_dicts(
        index_by(info, index_by="jobId", value_from_key="jobName") for info in jobs_info
    )
...
def index_commands_by_name(submitted):
    return {key: val["containerOverrides"]["command"] for key, val in submitted.items()}
...
def test_export(
    ...
    load_tasks_mock.assert_called_once_with(
        cmdr=commander_mock.__enter__(),
        name="train",
        mode=mode,
        lazy_import=False,
        task_name=None,
    ...
    submitted = index_submit_job_by_task_name(boto3_mock.submit_job.call_args_list)
    ...
    assert {j["jobName"] for j in jobs_info} == {
        "features",
        "fit",
        "get",
        "petal-area",
...
def test_export_multiple_images_job_details(
    ...
    NAME_CLEAN = "your-repository/name-clean-ploomber:latest"
    NAME_PLOT = "your-repository/name-plot-ploomber:latest"
    expected = {
        "arn:aws:batch:us-east-1:123456789012:job-definition/"
        "multiple_requirements_project-uuid4:1": "your-repository/name:latest",
        "arn:aws:batch:us-east-1:123456789012:job-definition/"
        "multiple_requirements_project-uuid4-clean-ploomber:1": NAME_CLEAN,
        "arn:aws:batch:us-east-1:123456789012:job-definition/"
        "multiple_requirements_project-uuid4-plot-ploomber:1": NAME_PLOT,
    }
...
def test_checks_the_right_spec(
    mock_batch,
    mock_docker_my_project_serve,
    monkeypatch,
    monkeypatch_docker_client,
    ...
    shutil.copy("src/my_project/pipeline.yaml", "src/my_project/pipeline.serve.yaml")
...
def mock_aws_batch(
    ...
    monkeypatch.setattr(batch.boto3, "client", lambda name, region_name: batch_mock)
    load_tasks_mock = Mock(wraps=commons.load_tasks)
    monkeypatch.setattr(commons, "load_tasks", load_tasks_mock)
...
@pytest.mark.parametrize(
    "task_resources, resource_requirements",
    [
        [
            {
                "fi*": {
                ...
                "get": {"vcpus": 4, "memory": 4096},
            },
            {
                "fit": [
                    {"value": "32", "type": "VCPU"},
                    {"value": "32768", "type": "MEMORY"},
                    {"value": "1", "type": "GPU"},
                ],
                "get": [
                    {"value": "4", "type": "VCPU"},
...
def test_custom_task_resources(
    mock_aws_batch, monkeypatch, task_resources, resource_requirements
):
    exporter = batch.AWSBatchExporter.new("soopervisor.yaml", "train")
    exporter.add()
    ...
    exporter = batch.AWSBatchExporter.load(
        path_to_config="soopervisor.yaml", env_name="train"
    )
    exporter.export(mode="incremental")

    submitted = index_submit_job_by_task_name(mock_aws_batch.submit_job.call_args_list)
...
def test_validates_task_resources(mock_aws_batch):
    ...
    with pytest.raises(ValidationError):
        batch.AWSBatchExporter.load(path_to_config="soopervisor.yaml", env_name="train")
...
def test_validates_names_in_task_resources(mock_aws_batch, monkeypatch, keys):
    ...
    assert "The following keys in the task_resources section" in str(excinfo.value)
    assert all([key in str(excinfo.value) for key in keys])


@pytest.fixture
def mock_aws_batch_lazy_load(
    mock_batch,
    mock_docker_my_project_serve,
    monkeypatch,
    monkeypatch_docker_client,
...
def test_lazy_load(mock_aws_batch_lazy_load, monkeypatch):
    exporter = batch.AWSBatchExporter.new("soopervisor.yaml", "train", lazy_import=True)
    exporter.add()
