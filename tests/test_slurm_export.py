...
from soopervisor.shell.export import SlurmExporter, commons, _script_name_for_task_name
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            "task-name",
            ("task-name.sh", "task-__.sh", "__-name.sh"),
            "task-name.sh",
            "ws",
        ],
...
def test_script_name_for_task_name(tmp_empty, name, files, match, workspace):
    ...
    assert _script_name_for_task_name(name, workspace) == Path(workspace, match)
...
def test_slurm_export_errors_if_missing_placeholder_in_template(
    ...
    exporter = SlurmExporter.new(path_to_config="soopervisor.yaml", env_name="serve")
    exporter.add()
...
def test_slurm_export_doesnt_require_lock_files(monkeypatch_slurm, tmp_sample_project):
...
def test_slurm_export_sample_project(monkeypatch_slurm, tmp_sample_project):
    ...
    load_tasks_mock.assert_called_once_with(
        cmdr=ANY, name="serve", mode="incremental", lazy_import=False, task_name=None
    )

    run_mock.assert_has_calls(
        [
            call(["sbatch", "--parsable", "_job.sh"], capture_output=True, check=True),
            call(
                [
                    "sbatch",
...
def test_slurm_export_sample_project_incremental(monkeypatch, tmp_sample_project):
    ...
    run_mock = Mock()
    run_mock.run = Mock(side_effect=[factory(b"0"), factory(b"1"), factory(b"2")])
    monkeypatch.setattr(commons, "load_tasks", load_tasks_mock)
    monkeypatch.setattr(export, "run", run_mock)
...
def test_slurm_export_sample_project_with_skip_docker(
    monkeypatch_slurm, tmp_sample_project, capsys
):
