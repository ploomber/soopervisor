...
def test_new_error_if_directory_with_name_exists(tmp_empty, method, kind):
    ...
    expected = (
        f"A {kind} named 'env' already exists in the "
...
def test_docker_config_pass_exclude(tmp_empty):
    ConcreteDockerConfig.new("soopervisor.yaml", "some_env", exclude=["some-dir"])
...
"""
    ...
    expected = (
        "Expected key 'some_env' in 'soopervisor.yaml' to contain "
...
def test_error_if_missing_backend_key_in_soopervisor(CLASS_, tmp_empty):
    ...
    assert (
        "Missing 'backend' key for section 'some_env' "
        "in 'soopervisor.yaml'. Add it and try again."
    ) == str(excinfo.value)
...
def test_error_if_invalid_backend_value(tmp_empty):
    ...
    expected = (
        "Invalid backend key for section 'some_env' in "
