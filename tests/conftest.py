...
def subprocess_run_mock(tester):
    def ret(cmd, capture_output):
        ...
        if capture_output:
            mock_subprocess_run_result.stdout.decode.return_value = result.decode(
                sys.stdout.encoding
            )
        else:
...
def _mock_commander_calls(monkeypatch, cmd, proj, tag, task=None):
    ...
    tester = _commander_tester.CommanderTester(
        ...
        return_value={
            ("docker", "run", f"{proj}{suffix}:{tag}", "python", "-c", cmd): b"True\n"
        },
    )
...
def backup_packaged_project():
    ...
    pkg_root = str(Path(importlib.util.find_spec("my_project").origin).parents[2])
